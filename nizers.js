 ...
 * @return void
 */
function pointnizer(inpId){
    var jlink = "xmpp:p@point.im?message;body=";

    var msgNumRegEx = /\B((#[a-z]+)(\/\d+)?)/igm;
    var nickRegEx = /\B(@[a-zA-Z0-9-.@_|]+)\b/gm;
    var nickRegEx2 = /\B@([a-zA-Z0-9-.@_|]+)\b/gm;
    var tagRegEx = /(\*\S+?)(?=<br>| \*)/igm;
    var youtubeRegEx = /<a.*?>(http:\/\/(?:www.)?youtube.com\/[a-zA-Z0-9?=&;#_]+)<\/a>/igm;
    var imageRegEx = /(<a[^<]*?>((?:http:\/\/)?(?:www\.)?[^\s]+?\/[^\s]+?(?:\.jpg|\.jpeg|\.gif|\.png))<\/a>)/igm;
    var avatarUrl = 'https://i.point.im/a/40/{0}.jpg';
    ...
    var msgNumRegEx2 = /\B(((#[a-z]+)(\/\d+)?)<\/a>)/igm;
    var msgNumControl = '(<a class="controls" title="Показать последние комментарии" href="' + jlink + '$3+">+</a>';
	msgNumControl += ' <a class="controls" title="Показать все комментарии" href="' + jlink + '$3++">++</a>';
    msgNumControl += ' <a class="controls" title="Подписаться на комментарии" href="' + jlink + 'S%20$3">S</a>';
    msgNumControl += ' <a class="controls" title="Отписаться от комментариев" href="' + jlink + 'U%20$3">U</a>';
    msgNumControl += ' <a class="controls" title="Удалить запись" href="' + jlink + 'D%20$2">D</a>';
    msgNumControl += ' <a class="controls" title="Рекомендовать запись" href="' + jlink + '!%20$3">!</a>)';
...
/** 
 * @description A nizer for LIJ (http://b.habahaba.im/ / http://lij.habahaba.im/).
 *
 * @return void
 */
function lijnizer(inpId){
    var jlink = "xmpp:lij.habahaba.im?message;body=";
    ...
    var youtubeRegEx = /<a.*?>(http:\/\/(?:www.)?youtube.com\/[a-zA-Z0-9?=&;#_]+)<\/a>/igm;
    var imageRegEx = /(<a[^<]*?>((?:http:\/\/)?(?:www\.)?[^\s]+?\/[^\s]+?(?:\.jpg|\.jpeg|\.gif|\.png))<\/a>)/igm;
    //var avatarUrl = 'http://b.habahaba.im/dmedia/img/avatars/HASH_96x96';		// ???
    ...
    msgNumControl += ' <a class="controls" title="Удалить запись" href="' + jlink + 'D%20$2">D</a>';
    msgNumControl += ' <a class="controls" title="Рекомендовать запись" href="' + jlink + '!%20$3">!</a>';
    msgNumControl += ' <a class="controls" title="XMPP-ссылка записи" href="' + jlink + 'C%20$3">C</a>)';
